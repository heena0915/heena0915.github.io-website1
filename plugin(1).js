...
tinymce.PluginManager.add('variable', function(editor) {
    ...
    function createHTMLVariable( value ) {
        ...
        return '<span class="' + className + ' mceNonEditable'  + '" data-original-variable="' + variable + '" contenteditable="false">' + cleanMappedValue + '</span>';
    }
    ...
    function stringToHTML(){
        ...
        tinymce.walk(editor.getBody(), function(n) {
            ...
            if (n.nodeType == 1) {
                ...
                if (original !== null) {
                    existingNodeList.push(n);
                }
            }

        }, 'childNodes');

        // loop over all exiting variable nodes and update the value
        for (var i = 0; i < existingNodeList.length; i++) {
            var existingNode = existingNodeList[i];
            var existingValue = existingNode.innerHTML;

            nodeValue = existingNode.getAttribute('data-original-variable');

            var cleanValue = cleanVariable(nodeValue);
            var cleanMappedValue = getMappedValue(cleanValue);

            if(existingValue !== cleanMappedValue){
              editor.dom.setHTML(existingNode, cleanMappedValue);
            }
        }
    ...
    editor.on('keyup', handleKeyEvent );
